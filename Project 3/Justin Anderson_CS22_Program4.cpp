...
print bubble and selection sort to 2 outside txt files
...
int main()
{
    ...
    //Execute the bubble sort (changed Bubble to bubblesort ja)
...
{  // not needed in program commented out (JA)
   ...
   // int intData1 = 0, intData2 = 0; //int values to hold converted strings
    ...
    do      //do loop makes sure the swap goes through the list at least once
    {
        ...
        for (int count = 0; count < (size - 1); count++) // for loop compares adjacent adultPop values in the whole array
        {
            ...
            int intData1 = stoi(CenDataArr[count].totPop);
            int intData2 = stoi(CenDataArr[count + 1].totPop);
           
            if (intData1 > intData2) //if the first value compared is greater, then it will swap it
            {
                // Swap elements using the temporary variable
                temp = CenDataArr[count];
                CenDataArr[count] = CenDataArr[count + 1];
                CenDataArr[count + 1] = temp;

                L_madeAswap = true; // Set the flag to true indicating a swap was made
                swapsMade++; // Increment swap counter
 ...
 **************************************************************/
 ...
 void selectionSort(CensusData CenDataArr[], int size)
 {
     ...
     CensusData minValue; // this creates an object to hold temporary object values
     
     for (int startScan = 0; startScan < (size - 1); startScan++)
     {
         
         minIndex = startScan;           //the first index starts at 0
         minValue = CenDataArr[startScan];
         for (int j = startScan + 1; j < size; j++)
         {
             int num1 = stoi(CenDataArr[j].totPop);//Convert the string variables into usable integer types for comparison (similar to bubbleSort conversion)
             int num2 = stoi(CenDataArr[minIndex].totPop); // INSERT YOUR CODE HERE FOR CONVERSION

             if (num1 < num2) //if the number we have is less than number 2
             {
                 // INSERT YOUR CODE HERE
                 minIndex = j; //updates the index of minimum value
                 minValue = CenDataArr[j];// chat gpt (Ja)
             }
         ...
         if (minIndex != startScan) {
             swap(CenDataArr[startScan], CenDataArr[minIndex]);
             swapsMade2++; //increment to remember number of swaps
         }
     ...
     cout << "Selection Sort has made " << swapsMade2 << " swaps which created the following array.";
 }
